...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (SecurityContextHolder.getContext().getAuthentication() == null) { // 인증 정보가 없으면 토큰 검증
            ...
            try {
                if (token != null) {
                    Claims claims = jwtTokenUtil.parseClaims(token); // 유효성 검사 + 파싱 한 번에

                    Authentication authentication = getAuthentication(request, claims.getSubject());

                    SecurityContextHolder.getContext().setAuthentication(authentication); // 인증 정보를 컨텍스트에 저장
                }
            } catch (ExpiredJwtException e) {
                request.setAttribute(EXCEPTION_ATTRIBUTE, EXPIRED_TOKEN); // 만료된 토큰
    ...
    // 인증 정보 생성
    private Authentication getAuthentication(HttpServletRequest request, String userId) {
